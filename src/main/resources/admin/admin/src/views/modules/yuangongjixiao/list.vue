		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yuangongjixiao','员工绩效统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						员工绩效统计
					</el-button>
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `yuangongjixiao/value/xingming/zongdefen`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: "yuangongjixiao/group/pingjiadengji",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
